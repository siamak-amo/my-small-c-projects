...
#define HEADER_FLAG(number) ((number) << 16 | HEADER_TEMPLATE)
#define HEADER_NUMBER(flg) ((flg) >> 16)
#define TEMPLATE_FLAG(x) ((x) & 0xFFFF)
...
typedef struct
{
   ...
   *  It must contain opt.wctx_len elements.
   */
  char **FUZZ;
} RequestContext;

/* used by fuzz_svprintf(), generated by gen_fuzz_cache(). */
struct template_cache
{
  /** flag:
   *  To determine the type of cache
   *  'u' for URL, 'b' for body, HEADER_FLAG('h') for headers
   */
  int flag;
  int stridx; /* index of FUZZ keywords */
  int widx; /* corresponding opt.wctx index */
...
typedef struct
{
  ...
  int header_count;
  int local_body_off;
  int local_off, local_cap; // offset of opt.wctx
} FuzzTemplate;
...
struct fword_ctx /* context of Fword. */
{
  Fword *fw;
 ...
 */
void fuzz_svprintf (struct strv *dst, const char *format, char **src,
...
int set_template_wlist (FuzzTemplate *t, enum template_op op,
                        struct fword_ctx *dst, void *param);
...
struct Opt
{
  ...
  struct fword_ctx *wctx; /* Dynamic array */
  int wctx_len; /* Length of @wctx */
...
#undef __ARG__
  ...
  else /* Multiple FUZZ keywords provided */
    {
      if (opt.Printf.color)
        ...
        fprintf (stream, "\n * FUZZ = [");
      Fprintarr (stream, "'%s'", ctx->FUZZ, opt.wctx_len);
      fprintf (stream, "]:\n");
    }
}
...
#define N 13
  char randstr[N];
  randstr[N-1] = '\0';
  for (ssize_t i = 0; i < opt.wctx_len; ++i)
    {
...
{
  Fword *fw = opt.wctx[0].fw;
  fw_export (ctx->FUZZ, fw);

  da_idx i=1, N = opt.wctx_len;
  for (; i < N; ++i)
...
{
  Fword *fw;
  size_t N = opt.wctx_len;

  static Fword *longest = NULL;
  /* Find the longest wordlist */
  if (! longest)
    {
      longest = opt.wctx[0].fw;
      for (size_t i=0; i < N; ++i)
        {
          if (opt.wctx[i].fw->total_count > longest->total_count)
            longest = opt.wctx[i].fw;
        }
    }
...
__register_context (RequestContext *dst)
{
  ...
  if (opt.fuzz_flag & URL_HASFUZZ)
    {
      sv_drop (&tmp);
      fuzz_svprintf (&tmp, template->URL, FUZZ, template->cache, &ss);
      ...
      if (opt.fuzz_flag & HEADER_HASFUZZ)
        {
          ...
          curl_slist_foreach (template->headers, h)
            {
              ++i;
              int flg = HEADER_NUMBER (template->cache[ss].flag);
              if (flg != i)
                {
                  /* this header does not have FUZZ keyword */
                  curl_slist_appd (req->headers, h->data);
                }
              else
                {
                  sv_drop (&tmp);
                  fuzz_svprintf (&tmp, h->data, FUZZ, template->cache, &ss);
  ...
  if (template->body)
    {
      if (opt.fuzz_flag & BODY_HASFUZZ)
        {
          sv_drop (&tmp);
          fuzz_svprintf (&tmp, template->body, FUZZ, template->cache, &ss);
...
//-- Cache generating functions --//
static inline int
__do_fuzz_cache (struct fword_ctx *fw, int ss, int fw_len,
                 struct template_cache *cache,
                 const char *begin, const char *start, const char **_end,
                 int flg)
{
      ...
      if ((end = strstr (p, tag)))
        {
          ...
          cache[i] = (struct template_cache) {
            .widx = count+ss,  .flag = flg,  .taglen = _taglen,
            .stridx = (int) (end - begin),
          };
          ++i, ++fw, p = end + _taglen;
        }
    }
...
static int
lookup_fw_idx (struct fword_ctx *haystack, int ss, int to, int needle_type)
{
  for (int i = ss; i < to; ++i)
    {
...
{
  struct fword_ctx *fw = opt.wctx;
  int fw_len = opt.wctx_len, off = 0;
  int __off = 0;
  ...
  if ((start = template->URL)  &&  (opt.fuzz_flag & URL_HASFUZZ))
    {
      ...
      off = __do_fuzz_cache (fw, __off, fw_len, template->cache,
                             start,start,NULL, 'u');
    }
  if (template->headers  &&  (opt.fuzz_flag & HEADER_HASFUZZ))
    {
      ...
      curl_slist_foreach (template->headers, h) {
        ++i;
        start = h->data;
        if ((__off = lookup_fw_idx (fw, i, fw_len, HEADER_FLAG(i))) < 0)
          continue;
        off =
          __do_fuzz_cache (fw, __off, fw_len, template->cache, start,start,NULL,
                           HEADER_FLAG (i));
      }
    }
  if ((start = template->body)  &&  (opt.fuzz_flag & BODY_HASFUZZ))
    {
      const char *end = start, *p = start;
      for (int i=0; end; p = end, i = off)
        {
          if ((__off = lookup_fw_idx (fw, i, fw_len, BODY_TEMPLATE)) < 0)
            break;
          off = __do_fuzz_cache (fw, __off, fw_len, template->cache, start,p,&end, 'b');
        }
    }
  assert (off == fw_len && "len(opt.wctx) != #FUZZ, broken logic.");
}
...
void
fuzz_svprintf (struct strv *dst, const char *format, char **src,
               const struct template_cache *cache, int *cache_ss)
{
  int ss = 0, to = 0;
  cache += *cache_ss; /* apply cache start offset */

  for (int i=0, flg = 0; i < opt.wctx_len;)
    {
...
static int
da_fw_lookup (struct fword_ctx *haystack, const char *needle)
{
  if (! needle)
    return -1;
  da_foreach (haystack, i)
    {
      if (! haystack[i].fw || !haystack[i].path)
        continue;
...
static struct fword_ctx
register_wlist (FuzzTemplate *t, const char *path, const char *tag)
{
  (void) t;
  struct fword_ctx res = {0};
  int idx = da_fw_lookup (opt.wctx, path);
  if (-1 == idx) /* New file */
    {
      res.fw = make_fw_from_path (path);
      res.path = path;
      if (! res.fw) /* could not use file, use dummy wordlist */
        res.fw = fw_dup (&dummy_fword);
    }
  else /* We have already opened this file */
    {
      res.fw = fw_dup (opt.wctx[idx].fw);
    }
  res.tag = tag;
  return res;
}
...
init_opt ()
{
  ...
  opt.wctx_len = da_sizeof (opt.wctx);
  if (opt.wctx_len == 0)
    {
      opt.eofuzz = true;
  ...
  for (size_t i = 0; i < opt.Rqueue.len; i++)
    {
      ...
      /* init FUZZ array */
      cap_bytes = (opt.wctx_len + 1) * sizeof (char *);
  ...
  switch (opt.mode)
    ...
    case MODE_PITCHFORK:
      ...
      for (da_idx i=0; i < opt.wctx_len; ++i)
        if (opt.wctx[i].fw->total_count > opt.progress.req_total)
          opt.progress.req_total = opt.wctx[i].fw->total_count;
      opt.load_next_fuzz = __next_fuzz_pitchfork;
      break;

    case MODE_SINGULAR:
      opt.progress.req_total = opt.wctx[0].fw->total_count;
...
int
set_template_flag (FuzzTemplate *t, enum template_op op,
                   struct fword_ctx *dst, void *param)
{
  (void) param;
  switch (TEMPLATE_FLAG (op))
    {
    case URL_TEMPLATE:
    case BODY_TEMPLATE:
      dst->type = op;
    ...
    case HEADER_TEMPLATE:
      dst->type = HEADER_FLAG (t->header_count);
      break;
                    ...
                    struct fword_ctx *dst, void *param)
{
          ...
          if (NULL == dst[i].fw)
            {
              ...
              set_template_flag (t, op, &dst[i], NULL);
              t->local_fuzz_count--;
              break;
            }
        }
  ...
  switch (TEMPLATE_FLAG (op))
    ...
    case URL_TEMPLATE:
      ...
      break;
    case BODY_TEMPLATE:
      tag_candidate = t->body + t->local_body_off;
      break;
    case HEADER_TEMPLATE:
  ...
  for (int i=0; i < t->local_cap; ++i)
    {
      ...
      if (! tag_candidate)
        break;
      if (dst[i].fw && dst[i].tag && strlen(dst[i].tag) >= strlen(tag))
        {
          tag_candidate += FUZZ_STR_LEN;
          continue;
        }
                  ...
                  if (NULL == dst[j].fw)
                    {
                      ...
                      if (dst[j].tag)
                        dst[j].tag = NULL;
                      dst[i] = (struct fword_ctx){0};
                      break;
                    }
                }
              assert ((resolved || (NULL == dst[i].fw)) && "tag swapping must success");
            }
          dst[i] = register_wlist (t, path, tag);
          set_template_flag (t, op, &dst[i], NULL);
          t->local_fuzz_count--, resolved++;
        }
      tag_candidate += FUZZ_STR_LEN; /* we know it's length >= FUZZ_STR */
    }
      ...
      if (WLIST_TEMPLATE != op  &&  0 < t->local_fuzz_count)
        {
          ...
          for (int i=0; 0 != t->local_fuzz_count; ++i)
            set_template_wlist (t, prev_op, opt.wctx, NULL);
        }
      break;
        ...
        if (t->local_fuzz_count > 0)
          {
            ...
            t->local_off = da_zallocate (opt.wctx, t->local_fuzz_count);
        ...
        if (opt.mode == MODE_SINGULAR)
          {
            ...
            if (0 == single++) /* the very only singular mode word-list */
              set_template_wlist (t, -1, opt.wctx, param);
            else
              {
...
singular mode only accepts one word-list", param);
        ...
        else
          {
            if (t->local_fuzz_count <= 0)
              ...
              set_template_wlist (t, prev_op, opt.wctx, param);
          }
      }
      return 0;
...
cleanup (int c, void *p)
{
  ...
  da_free (opt.filters);
  da_foreach (opt.wctx, i) {
    fw_free (opt.wctx[i].fw);
    if (opt.mode == MODE_SINGULAR) break;
  }
  da_free (opt.wctx);
  /* Template cleanup */
...
#define Return(n) return (n == SHOULD_EXIT) ? EXIT_SUCCESS : EXIT_FAILURE
  ...
  for (int i=0; i < opt.wctx_len; ++i)
    {
      struct fword_ctx *w = &opt.wctx[cache[i].widx];
      fprintf (stderr,
